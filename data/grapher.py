...
workload = (config.get('DEFAULT', 'workload'),)
...
def bttime():
    ...
    plt.gcf().text(0.005, 0.005, str(zip(names, config)))
    util.reset_axis()
    #plt.plot(timestamps, times)
    sns.lmplot("timestamps", "times", data=timedf)
...
def latmeanbw():
    ...
    normbws = np.array(cols_dict['bandwidths']) 
